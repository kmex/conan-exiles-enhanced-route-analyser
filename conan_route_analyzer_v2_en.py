#!/usr/bin/env python3
"""
Conan Exiles - Route Analyzer v2
Features: Real-time chart, Jitter History, ISP Comparison, Copy IP button,
Saved Server List, Windows Notifications
...
SERVERS_FILE = Path.home() / "Documents" / "ConanRoutes" / "servers.json"
HISTORY_FILE = Path.home() / "Documents" / "ConanRoutes" / "history.json"
...
def run_cmd(cmd, timeout=40):
    try:
        ...
        return (res.stdout or res.stderr or "No output.").strip()
...
def phase_discover(ip, base_port, log):
    log("[Phase 3] UDP Port Discovery...", CYAN)
    ...
    for p in ports:
        ...
        try:
            ...
            msg = f"[OK] Port {p:<5} responded in {rtt:>6.1f}ms -> {len(data)} bytes"
            responding.append(p)
            log(msg, GREEN)
        except socket.timeout:
            msg = f"[--] Port {p:<5} TIMEOUT"
            log(msg)
        except Exception as e:
            msg = f"[ER] Port {p:<5} {e}"
            log(msg, RED)
...
def phase_jitter(ip, port, count, log, on_packet=None, stop_event=None):
    log(f"[Phase 4] Stress Test on Port {port} ({count} packets)...", CYAN)
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.settimeout(0.5)
    latencies = []
    lost = 0
    ...
    if not latencies:
        return None, "100% LOSS - No response.", "CRITICAL"
    ...
    if loss_pct >= 2.0:
        diag = "BAD - Packet Loss detected!";           color = "BAD"
    elif avg_jitter >= JITTER_WARNING:
        diag = "BAD - High Jitter (stuttering/teleport)"; color = "BAD"
    elif avg_jitter >= JITTER_GOOD:
        diag = "WARNING - Moderate Jitter";              color = "WARNING"
    else:
        diag = "GOOD - Stable connection";               color = "GOOD"
    ...
    text = (
        f"Packets Sent    : {count}\n"
        f"Packets Lost    : {lost} ({loss_pct:.1f}%)\n"
        f"Min Latency     : {min_rtt:.1f} ms\n"
        f"Avg Latency     : {avg_rtt:.1f} ms\n"
        f"Max Latency     : {max_rtt:.1f} ms\n"
...
class JitterChart(tk.Canvas):
    ...
    def _redraw(self):
        ...
        if not all_vals:
            self.create_text(W // 2, H // 2, text="Waiting for packets...", fill=FG2)
            return
        ...
        if self.data_rtt:
            self.create_text(W - pad, pad,
                             text=f"RTT {self.data_rtt[-1]:.0f}ms  Jitter {self.data_jitter[-1]:.0f}ms",
...
class App:
    def __init__(self, root):
        self.root       = root
        self.root.title("Conan Exiles - Route Analyzer v2")
    ...
    def _build_ui(self):
        ...
        tk.Label(top, text="Local IP:", bg=BG2, fg=FG2, font=("Consolas", 9)).grid(row=0, column=0, padx=(12, 4))
        ...
        tk.Button(top, text="Copy", bg=BG3, fg=FG, relief="flat",
                  font=("Consolas", 8), command=self._copy_ip).grid(row=0, column=2, padx=(0, 20))

        tk.Label(top, text="Server IP:", bg=BG2, fg=FG2, font=("Consolas", 9)).grid(row=0, column=3, padx=(0, 4))
        ...
        tk.Label(top, text="Port:", bg=BG2, fg=FG2, font=("Consolas", 9)).grid(row=0, column=5, sticky="e", padx=(0, 4))
        ...
        # Tab 1: Live Test + Chart
        tab_test = tk.Frame(nb, bg=BG)
        nb.add(tab_test, text=" Live Test ")
        ...
        nb.add(tab_hist, text=" Jitter History ")
        tab_hist.columnconfigure(0, weight=1)
        tab_hist.rowconfigure(0, weight=1)

        cols = ("Date/Time", "Label", "Server", "Avg Jitter", "Max Peak", "Loss%", "Diagnostic")
        ...
        self.tree_hist.column("Diagnostic", width=260)
        ...
        tk.Button(btn_frame, text="Clear History", bg=BG3, fg=RED, relief="flat",
                  font=("Consolas", 8), command=self._clear_history).pack(side="left", padx=8)

        # Tab 3: Saved Servers
        tab_srv = tk.Frame(nb, bg=BG)
        nb.add(tab_srv, text=" Saved Servers ")
        ...
        self.tree_srv.column("Name", width=250)
        self.tree_srv.column("IP",   width=160, anchor="center")
        self.tree_srv.column("Port", width=80,  anchor="center")
        ...
        tk.Button(btn_srv, text="+ Add Current", bg=GREEN, fg="white", relief="flat",
                  font=("Consolas", 8), command=self._add_server).pack(side="left", padx=8)
        tk.Button(btn_srv, text="- Remove",      bg=RED,   fg="white", relief="flat",
                  font=("Consolas", 8), command=self._remove_server).pack(side="left", padx=4)
        tk.Label(btn_srv, text="Double-click to load server", bg=BG, fg=FG2,
    ...
    def _copy_ip(self):
        ...
        self.lbl_status.config(text=f"IP {ip} copied to clipboard!", fg=GREEN)
    ...
    def _start_test(self):
        ...
        label = self.var_label.get().strip().upper() or "TEST"
        if not ip or not port:
            messagebox.showerror("Error", "Please fill in the IP and Port.")
            return
        try:
            port = int(port)
        except ValueError:
    ...
    def _stop_test(self):
        self.stop_event.set()
        self.lbl_status.config(text="Stopping test...", fg=ORANGE)
    ...
    def _run_test(self, ip, port, label):
        ...
        log("=" * 60, YELLOW)
        log(f" CONAN EXILES ROUTE ANALYZER v2 | {label} | {ip}:{port}", YELLOW)
        log(f" {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", YELLOW)
        log("=" * 60, YELLOW)

        status("Phase 1: ICMP Ping...", CYAN)
        ...
        status("Phase 2: Traceroute...", CYAN)
        ...
        status("Phase 3: Port Discovery...", CYAN)
        active_ports, r3 = phase_discover(ip, port, log)

        status("Phase 4: Stress Test (100 packets)...", CYAN)
        log("\n[Phase 4] Jitter Stress Test...", CYAN)

        if not active_ports:
            log("FAILED: No port responded.", RED)
            self._finish_test(report, ts, ip, port, label, None)
            return
    ...
    def _finish_test(self, report, ts, ip, port, label, stats):
        ensure_dirs()
        outdir  = Path.home() / "Documents" / "ConanRoutes"
        outpath = outdir / f"report_{label}_{ip}_{ts}.txt"
        outpath.write_text("\n".join(report), encoding="utf-8")

        color_map = {"GOOD": GREEN, "WARNING": ORANGE, "BAD": RED, "CRITICAL": RED}
        if stats:
            ...
            self.root.after(0, self.lbl_status.config,
                            {"text": f"Done: {diag} | Report: {outpath.name}", "fg": c})

            if stats["color"] in ("BAD", "CRITICAL"):
                notify_windows("Conan - Issue Detected",
                               f"{label} | Jitter: {jitter_val:.1f}ms | {diag}")
        else:
            self.root.after(0, self.lbl_status.config,
                            {"text": "Test cancelled or no response.", "fg": ORANGE})
    ...
    def _refresh_history(self):
        ...
        for e in reversed(self.history):
            tag    = e.get("color") or ("BAD" if e["jitter"] >= JITTER_WARNING else
                                        "WARNING" if e["jitter"] >= JITTER_GOOD else "GOOD")
            dt     = e["ts"]
        ...
        for tag_name, col in [("GOOD", GREEN), ("WARNING", ORANGE), ("BAD", RED), ("CRITICAL", RED)]:
    ...
    def _add_server(self):
        ...
        if not ip or not port:
            messagebox.showerror("Error", "Please fill in the IP and Port.")
            return
        name = simpledialog.askstring("Server Name", "Enter a name to save:", initialvalue=ip)
        if not name:
            return
    ...
    def _load_server(self, event=None):
        ...
        self.lbl_status.config(text=f"Server loaded: {s['name']} ({s['ip']}:{s['port']})", fg=GREEN)
